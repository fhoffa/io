...
# Query to generate a mapping of position -> move probability
...
ORDER BY
  ...
  section_x
EOF`
MOVE_PROBABILITY=$DATASET.move_probabilities_position
...
# Query to generate a mapping of position -> shot probability
...
ORDER BY
  ...
  section_x
EOF`
SHOT_PROBABILITY=$DATASET.shot_probabilities_position
...
# Query to generate a mapping of position -> pass probability
...
ORDER BY
  ...
  section_x
EOF`
PASS_PROBABILITY=$DATASET.pass_probabilities_position
...
# Query to generate a mapping of position -> save probability
...
ORDER BY
  ...
  section_x
EOF`
SAVE_PROBABILITY=$DATASET.save_probabilities_position
...
# Query to generate a mapping of position -> intercept probability
...
ORDER BY
  ...
  section_x
EOF`
INTERCEPT_PROBABILITY=$DATASET.intercept_probabilities_position
...
# Query to generate a mapping of position -> tackle probability
...
ORDER BY
  ...
  section_x
EOF`
TACKLE_PROBABILITY=$DATASET.tackle_probabilities_position
...
# Query to generate a mapping of position -> foul probability
...
FROM [$GAME_SUMMARY] WHERE
  ...
  GROUP BY position, section_x, section_y
EOF`
FOUL_PROBABILITY=$DATASET.foul_probabilities_position
...
# Query to generate a mapping of position -> out probability
...
FROM [$GAME_SUMMARY] WHERE
  ...
  GROUP BY position, section_x, section_y
EOF`
OUT_PROBABILITY=$DATASET.out_probabilities_position
...
# Query to generate a mapping of position -> defense probability
...
FROM [soccer.game_summary_0]
  GROUP BY position, section_x, section_y
EOF`
DEFENSE_PROBABILITY=$DATASET.defense_probabilities_position
...
# Query to generate a mapping of position -> offense probability
...
FROM [soccer.game_summary_0]
  GROUP BY position, section_x, section_y
EOF`
OFFSENSE_PROBABILITY=$DATASET.offsense_probabilities_position
